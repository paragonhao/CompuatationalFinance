...
double OptionPricing::FixedStrikeLookBackCall(const double &r, const double &T, const double &s0, const double &strike, const double &sigma,
                                              const int & num, const int & steps, const double & delta){

    double payoff = 0;

    for(int i = 0; i< num; i++){
        double * dwt = RandomGenerator::wienerProcess(delta, steps, rand());
        ...
        for(int j=0; j<steps; j++){
            path[j+1] = path[j]* exp((r - 0.5 * sigma * sigma)* delta + sigma * dwt[j]);
            maxS = (path[j+1] > maxS)?path[j+1]:maxS;
        }
        payoff += (maxS - strike >0)?(maxS - strike ):0;

    }
    return payoff/num;
}

double OptionPricing::FixedStrikeLookBackPut(const double &r, const double &T, const double &s0, const double &strike, const double &sigma,
                                              const int & num, const int & steps, const double & delta){

    double payoff = 0;
    static const int32_t maxNum = numeric_limits<std::int32_t>::max();
    ...
    for(int i = 0; i< num; i++){
        ...
        double minS = double(maxNum);

        for(int j=0; j<steps; j++){
            path[j+1] = path[j]* exp((r - 0.5 * sigma * sigma)* delta + sigma * dwt[j]);
            minS = (path[j+1] < minS)?path[j+1]:minS;
        }
        payoff += (strike - minS >0)?(strike - minS):0;
